...
import argparse
import numpy as np
# yolo_weights ='yolo3-tiny.weights'
# yolo_cfg = 'yolo3-tiny.cfg'
...
def dectection_video_file(webcam, video_path, yolo_weights, yolo_cfg, coco_names, confidence_threshold,
                          nms_threshold):
    net, classes, output_layers = yolov3(yolo_weights, yolo_cfg, coco_names)
    colors = np.random.uniform(0, 255, size=(len(classes), 3))
...
if __name__ == '__main__':
    ...
    ap.add_argument('--camera', help='To use the live feed from web-cam', type=bool, default=True)
    ap.add_argument('--weights', help='Path to model weights', type=str, default='yolov3-tiny.weights')
    ap.add_argument('--configs', help='Path to model configs',type=str, default='yolov3-tiny.cfg')
